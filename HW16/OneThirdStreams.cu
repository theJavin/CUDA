...
int main()
{
	...
	for(int i = 0; i < ENTIRE_DATA_SET; i += DATA_CHUNKS)
	{
		...
		*/
		...
		cudaMemcpyAsync(A0_GPU, A_CPU + i/3, DATA_CHUNKS*sizeof(float), cudaMemcpyHostToDevice, Stream0);
        myCudaErrorCheck(__FILE__, __LINE__);
		cudaMemcpyAsync(A1_GPU, A_CPU + 2*(i/3), DATA_CHUNKS*sizeof(float), cudaMemcpyHostToDevice, Stream1);
        myCudaErrorCheck(__FILE__, __LINE__);
		cudaMemcpyAsync(A2_GPU, A_CPU + 3*(i/3), DATA_CHUNKS*sizeof(float), cudaMemcpyHostToDevice, Stream2);
        myCudaErrorCheck(__FILE__, __LINE__);

        cudaMemcpyAsync(B0_GPU, B_CPU + i/3, DATA_CHUNKS*sizeof(float), cudaMemcpyHostToDevice, Stream0);
        myCudaErrorCheck(__FILE__, __LINE__);
		cudaMemcpyAsync(B1_GPU, B_CPU + 2*(i/3), DATA_CHUNKS*sizeof(float), cudaMemcpyHostToDevice, Stream1);
        myCudaErrorCheck(__FILE__, __LINE__);
		cudaMemcpyAsync(B2_GPU, B_CPU + 3*(i/3), DATA_CHUNKS*sizeof(float), cudaMemcpyHostToDevice, Stream2);
        myCudaErrorCheck(__FILE__, __LINE__);

        trigAdditionGPU<<<GridSize, BlockSize, 0, Stream0>>>(A0_GPU, B0_GPU, C0_GPU, DATA_CHUNKS/3);
        trigAdditionGPU<<<GridSize, BlockSize, 0, Stream1>>>(A1_GPU, B1_GPU, C1_GPU, DATA_CHUNKS/3);
        trigAdditionGPU<<<GridSize, BlockSize, 0, Stream2>>>(A2_GPU, B2_GPU, C2_GPU, DATA_CHUNKS/3);

        cudaMemcpyAsync(C_CPU + i/3, C0_GPU, DATA_CHUNKS*sizeof(float), cudaMemcpyDeviceToHost, Stream0);
        myCudaErrorCheck(__FILE__, __LINE__);
        cudaMemcpyAsync(C_CPU + 2*(i/3), C1_GPU, DATA_CHUNKS*sizeof(float), cudaMemcpyDeviceToHost, Stream1);
        myCudaErrorCheck(__FILE__, __LINE__);
        cudaMemcpyAsync(C_CPU + 3*(i/3), C2_GPU, DATA_CHUNKS*sizeof(float), cudaMemcpyDeviceToHost, Stream2);
        myCudaErrorCheck(__FILE__, __LINE__);	
